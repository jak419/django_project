...
def index(request):
    ...
    product_subcategories = ProductSubcategory.objects.annotate(
    ...
    if request.user.is_authenticated:
        try:
            ...
            shipped_orders = Orders.objects.filter(
                customer=customer, status='s').order_by('-order_date')[:5]
...
class ProductListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['product_subcategories'] = ProductSubcategory.objects.annotate(
            total_products=Count('product'))
        return context
...
def create_order(request):
    ...
    product_prices = json.dumps({str(product['id']): (float(
        product['list_price']) if product['list_price'] else 0) for product in products})
...
class AllOrdersListView(PermissionRequiredMixin, ListView):
    ...
    # Ensuring only authorized users can view this
    permission_required = 'catalog.can_mark_shipped'
